...
void Tmc2209::begin(uint8_t node, uint32_t baud) {
  node_ = node;
  // Serial1, ne Serial - konzole je nativni USB CDC. UART0 (IO43/IO44) neni
  // na desce nikam zapojeny.
...
bool Tmc2209::configure(uint16_t run_ma, uint8_t hold_pct, uint16_t microsteps) {
  ...
  // CHOPCONF jde PRVNI, a to zamerne (viz ../TMC2209_MS1_MS2.md, HW-00201):
  // dokud je GCONF bit7 (mstep_reg_select) nula, bere driver mikrokrok z pinu
  // MS1/MS2. V okamziku, kdy se ten bit prehodi, zacne platit hodnota
  // v registru - takze v nem uz musi byt ta spravna. V obracenem poradi by
  // driver mezitim jel podle toho, co v MRES zbylo (po resetu 0, tedy 1/256),
  // a na stejny pocet pulzu by se hridel otocila 16x mene.
  //
