...
struct DrvStatus {
  ...
  bool ola = false;    // rozpojena faze A (hlasi ji i stojici motor)
  bool olb = false;    // rozpojena faze B (dtto)
  bool stst = false;   // motor stoji
  uint8_t cs_actual = 0;  // skutecne pouzity CS - 0 znamena, ze netece proud

  // Chyba, na kterou se ma zastavit. ola/olb v tom NEJSOU: hlasi je i zdravy
  // stojici motor pri stealthChop. otpw taky ne, to je jen predzvest.
