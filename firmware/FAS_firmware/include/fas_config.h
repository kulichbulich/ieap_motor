// Vsechna nastaveni FAS firmware na jednom miste.
//
// Nic z toho se za behu nemeni - firmware nema konzolove prikazy, jen vypisuje.
// Zmena = uprava tohohle souboru, prelozit a nahrat. Ladit hodnoty za behu
// (proud, rychlost, mikrokrok, jiny motor) se da v ../testing_firmware, test T09.
...
// --- ktery motor -----------------------------------------------------------
// Slot driveru, 0..3 (BOB-0..3 na desce). Adresa driveru na UART sbernici je
// TMC_ADDR[MOTOR], koncovy spinac PIN_ENDSWITCH[MOTOR], STEP pin PIN_STEP[MOTOR].
// Vychozi 3 (ctvrty slot) - tam je zapojeny motor, na kterem se oziva deska.
...
// Zrychleni v otackach za sekundu na sekundu. Rampa se vejde do pohybu: pri
// 60 rpm a 4 ot/s^2 se na plnou rychlost rozjede za 0.25 s (~ 1/8 otacky).
...
// Cekat po odpoctu jeste na Enter z konzole? true = deska se sama nerozjede,
// coz je bezpecnejsi na stole, ale nepouzitelne pri behu bez pripojeneho PC.
static constexpr bool WAIT_FOR_ENTER = false;
// Hlidat koncovy spinac motoru a pri sepnuti pohyb zastavit. Kdyz je spinac
// v klidu sepnuty (nebo neni zapojeny a chybi pull-up), hlidani se pri startu
// samo vypne - jinak by se firmware nikdy nerozjel.
static constexpr bool ENDSTOP_GUARD = true;
// Jakykoli znak z konzole behem jizdy pohyb prerusi (softwarova stop klavesa).
static constexpr bool STOP_ON_KEYPRESS = true;
